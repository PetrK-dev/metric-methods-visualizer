...
describe('LaesaRangeAlgorithm', () => {
  ...
  beforeEach(() => {
    ...
    // Nastavení prvních dvou bodů jako pivoty (to je klíčový rozdíl oproti AESA)
  ...
  test('execute finds all points within the radius', async () => {
    ...
    // Spuštění algoritmu
    const iterator = LaesaRangeAlgorithm.execute(database, distanceMatrix, queryPoint, radius);
